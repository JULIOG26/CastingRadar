...
class CastingFilter:
    ...
    def filter(self, castings):
        ...
        for casting in castings:
            ...
            puntuacion += ROLE_SCORE.get(datos["papel"], 0)
            puntuacion += PRODUCTION_SCORE.get(produccion, 0)
            puntuacion += REMUNERATION_SCORE.get(datos["remunerado"], 0)
            puntuacion += RIGHTS_SCORE.get(datos["derechos"], 0)
            puntuacion += LOCATION_SCORE.get(datos["ubicacion"], 0)
            puntuacion += SEX_SCORE.get(datos["sexo"], 0)
